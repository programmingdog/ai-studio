  ...
  return <section className="section-card software-download-config">
    ...
    {loading ? <div className="loading-card"><span className="spinner" />正在读取安装包配置…</div> : config && <form onSubmit={save}>
      ...
      <fieldset className="download-platform-grid" disabled={saving}>
        <article className={config.windows_download_enabled ? "enabled" : ""}><label className="download-version-switch"><input type="checkbox" checked={config.windows_download_enabled} onChange={event => setConfig({ ...config, windows_download_enabled: event.target.checked })} /><span><strong>开放 Windows 版本</strong><small>开启后在下载页面显示 Windows 下载按钮</small></span></label><label>Windows 安装包下载地址<input type="url" maxLength={1000} value={config.windows_download_url} onChange={event => setConfig({ ...config, windows_download_url: event.target.value })} placeholder="https://download.example.com/逐梦帧-setup.exe" /><small>支持 EXE、MSI 或完整 HTTPS 下载链接。</small></label></article>
        <article className={config.macos_download_enabled ? "enabled" : ""}><label className="download-version-switch"><input type="checkbox" checked={config.macos_download_enabled} onChange={event => setConfig({ ...config, macos_download_enabled: event.target.checked })} /><span><strong>开放 macOS 版本</strong><small>开启后在下载页面显示 macOS 下载按钮</small></span></label><label>macOS 安装包下载地址<input type="url" maxLength={1000} value={config.macos_download_url} onChange={event => setConfig({ ...config, macos_download_url: event.target.value })} placeholder="https://download.example.com/逐梦帧.dmg" /><small>支持 DMG、PKG 或完整 HTTPS 下载链接。</small></label></article>
      </fieldset>
