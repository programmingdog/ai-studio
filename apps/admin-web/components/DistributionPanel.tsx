...
type Row = { id: string; user_id?: string; beneficiary_id?: string; inviter_id?: string; invited_user_id?: string; user_login_name?: string; inviter_login_name?: string; invited_login_name?: string; payer_id?: string; amount_fen?: number; base_amount_fen?: number; rate_bps?: number; level?: number; credits?: number; source_credits?: number; cost_credits?: number | null; profit_credits?: number | null; cny_per_credit?: number | null; source_capability?: string; status?: string; status_note?: string; created_at: string; review_note?: string; alipay_trade_no?: string; withdrawal_id?: string; payment_order_id?: string; consumption_record_id?: string; qualified_payment_order_id?: string };
...
export function DistributionConfigPanel({ token }: { token: string }) {
  ...
  return <section className="section-card distribution-config"><header><div><span className="kicker">REFERRALS & COMMISSION</span><h2>分销与邀请配置</h2><p>最多两级，仅按下级图片、视频生成的利润积分计提。配置变更仅影响后续入账，不重算历史记录。</p></div></header>
    ...
    {!config ? <p>正在读取配置…</p> : <form onSubmit={save}><fieldset disabled={busy}>
      ...
      <label>客户端分润提示<textarea rows={4} maxLength={1000} value={config.commission_notice || ""} onChange={e => setConfig({ ...config, commission_notice: e.target.value })} placeholder="例如：图片和视频生成产生的有效积分消耗，将按当前比例计入分润。" /><small>保存后显示在客户端“分润与提现”页面，最多 1000 个字符。</small></label>
      ...
      <p>利润积分 = 本次实际扣费积分 − 对应模型成本积分，最低为 0；按任务创建时“1 积分对应人民币”的配置换算，再乘各级分润比例。每级最终金额不足 1 分舍去。仅图片和视频生成参与，其他模型消耗及充值不计提。</p>
      <p>两项比例合计不得超过 100%。仅限制两级不代表模式自动合规，请在启用前完成业务合规审核。</p>
...
export function DistributionRecordsPanel({ token, kind }: { token: string; kind: "commissions" | "withdrawals" | "payouts" | "rewards" }) {
  ...
  return <section className="section-card table-card distribution-records"><header><div><span className="kicker">DISTRIBUTION LEDGER</span><h2>{{ commissions: "分润记录", withdrawals: "提现申请", payouts: "打款记录", rewards: "邀请奖励记录" }[kind]}</h2><p>资金记录不可直接修改或删除。提现审核不会自动转账，实际支付由管理员在支付宝中完成。</p></div><button className="secondary" disabled={loading} onClick={() => void load()}>刷新</button></header>
    ...
    <div className="table-scroll"><table><thead><tr><th>记录 / 用户</th><th>金额 / 奖励</th><th>状态 / 计提依据</th><th>时间</th><th>操作 / 关联记录</th></tr></thead><tbody>{data?.items.map(row => <tr key={row.id}>
      <td><strong>{row.id}</strong>{kind === "rewards" ? <><small className="cell-note">邀请人登录名：{row.inviter_login_name || row.inviter_id || "—"}</small><small className="cell-note">被邀请人登录名：{row.invited_login_name || row.invited_user_id || "—"}</small></> : <><small className="cell-note">用户登录名：{row.user_login_name || row.user_id || row.beneficiary_id || "—"}</small><small className="cell-note">用户 ID：{row.user_id || row.beneficiary_id || "—"}</small></>}</td>
      <td>{kind === "rewards" ? `${row.credits} 积分` : money(row.amount_fen)}</td>
      <td>{row.level ? <>{row.profit_credits != null && row.cny_per_credit != null ? `${row.level === 1 ? "直接" : "间接"} ${Number(row.profit_credits)} 利润积分 × ¥${Number(row.cny_per_credit)}/积分 × ${Number(row.rate_bps) / 100}%` : `${row.level === 1 ? "直接" : "间接"} ${Number(row.rate_bps) / 100}% × ${money(row.base_amount_fen)}（历史）`}{row.source_capability && <small className="cell-note">{row.source_capability === "IMAGE_GENERATION" ? "图片生成" : "视频生成"}扣费 {Number(row.source_credits || 0)} 积分{row.cost_credits != null && `，模型成本 ${Number(row.cost_credits)} 积分`}</small>}</> : kind === "rewards" ? rewardStatusName(row.status) : statusName(row.status)}{(row.review_note || row.status_note) && <small className="cell-note">{row.review_note || row.status_note}</small>}</td>
      <td>{formatDatabaseDateTime(row.created_at)}</td>
      <td>{kind === "withdrawals" ? <div className="table-actions">{["PENDING", "APPROVED"].includes(row.status || "") && <button className="secondary" disabled={openingPayee} onClick={() => setReviewing(row)}>审核</button>}<button className="secondary" disabled={openingPayee} onClick={() => void payee(row)}>{row.status === "APPROVED" ? "领取打款任务" : row.status === "PROCESSING" ? "查看打款任务" : "查看收款资料"}</button></div> : <small>{row.alipay_trade_no || row.consumption_record_id || row.payment_order_id || row.qualified_payment_order_id || row.invited_user_id}{row.withdrawal_id && <span className="cell-note">提现单：{row.withdrawal_id}</span>}</small>}</td>
