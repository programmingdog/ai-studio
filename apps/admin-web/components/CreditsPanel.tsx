...
const sectionCopy = {
  packages: { eyebrow: "CREDIT PACKAGES", title: "积分套餐", note: "配置套餐积分、额外赠送积分与独立售价；赠送积分不会抵扣售价。" },
...
function PackageModal({ token, item, onClose, onSaved }: { token: string; item: CreditPackage | null; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  return <div className="modal-backdrop"><form className="modal" onSubmit={submit}><header><div><span className="kicker">{item ? "EDIT PACKAGE" : "NEW PACKAGE"}</span><h2>{item ? "编辑积分套餐" : "新增积分套餐"}</h2></div><button type="button" onClick={onClose}>×</button></header><div className="two-columns"><label>套餐名称<input value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} required /></label><label>套餐 Code<input value={form.code} onChange={(event) => setForm({ ...form, code: event.target.value.toLowerCase() })} placeholder="starter-500" required /></label></div><label>套餐说明<input value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} /></label><div className="three-columns"><label>套餐积分（不含赠送）<input type="number" min="1" step="1" value={form.base_credits} onChange={(event) => setForm({ ...form, base_credits: event.target.value })} required /></label><label>额外赠送积分<input type="number" min="0" step="1" value={form.bonus_credits} onChange={(event) => setForm({ ...form, bonus_credits: event.target.value })} required /></label><label>套餐原价（元，赠送不抵扣）<input type="number" min="0.01" step="0.01" value={form.price_yuan} onChange={(event) => setForm({ ...form, price_yuan: event.target.value })} required /></label></div><div className="three-columns"><label>币种<input value={form.currency} maxLength={3} onChange={(event) => setForm({ ...form, currency: event.target.value.toUpperCase() })} required /></label><label>状态<select value={form.status} onChange={(event) => setForm({ ...form, status: event.target.value })}><option value="ACTIVE">启用</option><option value="DISABLED">停用</option></select></label><label>排序值<input type="number" min="0" step="1" value={form.sort_order} onChange={(event) => setForm({ ...form, sort_order: event.target.value })} required /></label></div>{error && <div className="form-error">{error}</div>}<footer><button type="button" className="secondary" onClick={onClose}>取消</button><button className="primary" disabled={saving}>{saving ? "保存中…" : "保存套餐"}</button></footer></form></div>;
}
...
function ConsumptionsManager({ token }: { token: string }) {
  ...
  const [showClearConfirm, setShowClearConfirm] = useState(false); const [clearing, setClearing] = useState(false); const [clearError, setClearError] = useState(""); const [clearedMessage, setClearedMessage] = useState("");
  ...
  const clearAll = async () => {
    if (clearing) return;
    setClearing(true); setClearError(""); setError(""); setClearedMessage("");
    try {
      const result = await apiRequest<{ deleted_count: number }>("/admin/credits/consumptions", { method: "DELETE", body: JSON.stringify({ confirmed: true, confirmation: "CLEAR_ALL_CREDIT_CONSUMPTIONS" }) }, token);
      setShowClearConfirm(false); setClearedMessage(`已清空 ${Number(result.deleted_count || 0).toLocaleString()} 条积分消耗记录。`); await load();
    } catch (reason) { setClearError(message(reason, "清空结果未确认，请刷新列表核对后再操作")); }
    finally { setClearing(false); }
  };
  return <section className="section-card table-card credit-table-card"><header><div><span className="kicker">CREDIT USAGE</span><h2>消耗记录</h2><p>支持模型任务、API 测试、人工录入和积分调整。</p></div><div className="record-header-actions"><span className="record-count">{rows?.length ?? 0} 条</span><button className="primary" disabled={!users.length} onClick={() => setEditing("new")}>新增消耗记录</button><button className="danger-button" disabled={!rows?.length || clearing} onClick={() => { setClearError(""); setShowClearConfirm(true); }}>清空全部记录</button></div></header>{error && <div className="form-error">{error}</div>}{clearedMessage && <div className="form-success" role="status">{clearedMessage}</div>}{!rows ? <div className="loading-card"><span className="spinner" />正在读取消耗记录…</div> : <div className="table-scroll"><table><thead><tr><th>记录号</th><th>用户</th><th>类型</th><th>模型/说明</th><th>消耗积分</th><th>状态</th><th>发生时间</th><th>操作</th></tr></thead><tbody>{rows.length ? rows.map((row) => <tr key={row.id}><td><code>{row.consumption_no}</code></td><td><strong>{row.user_name || "未命名"}</strong><small className="cell-note">{row.user_email || row.user_id}</small></td><td>{row.category}</td><td>{row.model_alias || row.description || "—"}<small className="cell-note">{row.model_code || row.task_id || ""}</small></td><td><strong className="credit-negative">-{row.credits_consumed.toLocaleString()}</strong></td><td><span className={`status ${tone(row.status)}`}>{row.status}</span></td><td>{formatDate(row.occurred_at)}</td><td><div className="table-actions"><button className="secondary" onClick={() => setEditing(row)}>编辑</button><button className="danger-button" onClick={() => void remove(row)}>删除</button></div></td></tr>) : <tr><td colSpan={8} className="empty-row">暂无积分消耗记录</td></tr>}</tbody></table></div>}{editing && <ConsumptionModal token={token} item={editing === "new" ? null : editing} users={users} onClose={() => setEditing(null)} onSaved={async () => { setEditing(null); await load(); }} />}{showClearConfirm && <ClearConsumptionsModal busy={clearing} error={clearError} onClose={() => { if (!clearing) setShowClearConfirm(false); }} onConfirm={() => void clearAll()} />}</section>;
}

function ClearConsumptionsModal({ busy, error, onClose, onConfirm }: { busy: boolean; error: string; onClose: () => void; onConfirm: () => void }) {
  return <div className="modal-backdrop" onMouseDown={(event) => { if (!busy && event.target === event.currentTarget) onClose(); }}><section className="modal destructive-confirm-modal" role="dialog" aria-modal="true" aria-labelledby="clear-consumptions-title" tabIndex={-1} onKeyDown={(event) => { if (!busy && event.key === "Escape") onClose(); }}><header><div><span className="kicker">IRREVERSIBLE ACTION</span><h2 id="clear-consumptions-title">清空全部积分消耗记录？</h2></div><button type="button" aria-label="关闭清空确认窗口" disabled={busy} onClick={onClose}>×</button></header><div className="destructive-warning"><strong>此操作永久生效，无法恢复。</strong><p>确认后将一次性删除数据库中的全部积分消耗展示记录，包括模型任务、API 测试、人工录入和积分调整记录。</p></div><ul className="destructive-consequences"><li>不会返还或补发任何积分，也不会改变用户当前积分余额。</li><li>不会删除积分账本流水、模型任务、套餐购买记录或用户资料。</li><li>历史消耗记录将无法继续在此页面查询；后续产生的新消耗仍会正常记录。</li><li>系统会在管理员审计日志中记录本次操作及删除数量。</li></ul>{error && <div className="form-error" role="alert">{error}</div>}<footer><button type="button" className="secondary" autoFocus disabled={busy} onClick={onClose}>取消</button><button type="button" className="danger-button danger-confirm-button" disabled={busy} onClick={onConfirm}>{busy ? "正在清空…" : "确认清空全部记录"}</button></footer></section></div>;
}
