      ...
      {selected ? <>
        ...
        {loadingModels ? <div className="loading-card"><span className="spinner" />正在读取模型…</div> : <div className="model-groups">{groupedModels.map(({ capability, items }) => <section key={capability} className="model-group">
          ...
          {items.length ? <div className="model-list">{items.map((model) => <article key={model.id}>
            ...
            <div className="model-pricing"><small>用户最终积分（小数向上取整）</small>{model.resolution_prices?.length ? model.resolution_prices.map((price) => <span className="resolution-price" key={price.resolution}><strong>{price.resolution}</strong><em>{price.final_credit_cost ?? price.credit_cost} 积分{model.capability === "VIDEO_GENERATION" && model.billing_unit !== "PER_REQUEST" ? " / 秒" : " / 次"}</em><small>消耗 {price.credit_cost} × 模型系数 {model.credit_multiplier}</small></span>) : <><strong>{model.final_credit_cost ?? model.credit_cost}</strong><span>{model.capability === "VIDEO_GENERATION" && model.billing_unit !== "PER_REQUEST" ? "积分 / 秒" : "积分 / 次"}</span><small>消耗 {model.credit_cost} × 模型系数 {model.credit_multiplier}</small></>}</div>
            {model.capability === "VIDEO_GENERATION" && model.video_duration_options?.length > 0 && <small>可选时长：{model.video_duration_options.join("、")} 秒</small>}
...
type ModelForm = {
  ...
  credit_cost: number; billing_unit: "PER_SECOND" | "PER_REQUEST"; video_duration_options: string;
  credit_multiplier: number; max_reference_images: number; supports_reference_video: boolean;
  supports_real_person: boolean;
...
function modelPayload(model: ProviderModel | ModelForm) {
  ...
  const durationOptions = "config" in model ? model.video_duration_options.trim().split(/[\s,，]+/).filter(Boolean).map(Number) : model.video_duration_options;
  if (model.capability === "VIDEO_GENERATION" && durationOptions.some((value) => !Number.isInteger(value) || value < 1 || value > 3600)) throw new Error("时长选项请填写 1～3600 的整数，例如 5, 10, 15");
  return {
    ...
    query_endpoint: model.query_endpoint || "", credit_cost: Number(model.credit_cost), billing_unit: model.capability === "VIDEO_GENERATION" ? model.billing_unit : "PER_REQUEST",
    video_duration_options: model.capability === "VIDEO_GENERATION" ? durationOptions : [], credit_multiplier: Number(model.credit_multiplier),
...
function ModelModal({ token, providerId, model, onClose, onSaved }: { token: string; providerId: string; model: ProviderModel | null; onClose: () => void; onSaved: () => Promise<void> }) {
  const [form, setForm] = useState<ModelForm>({
    ...
    credit_cost: model?.credit_cost || 1, billing_unit: model?.billing_unit || "PER_SECOND",
    video_duration_options: model?.video_duration_options?.join(", ") || "",
    credit_multiplier: model?.credit_multiplier ?? 1, max_reference_images: model?.max_reference_images || 0,
  ...
  return <div className="modal-backdrop"><form className="modal model-modal" onSubmit={submit}><header><div><span className="kicker">{model ? "EDIT MODEL" : "NEW MODEL"}</span><h2>{model ? "编辑大模型" : "新增大模型"}</h2></div><button type="button" onClick={onClose}>×</button></header>
    ...
    {form.capability === "VIDEO_GENERATION" && <div className="two-columns"><label>视频计费方式<select value={form.billing_unit} onChange={(event) => setForm({ ...form, billing_unit: event.target.value as ModelForm["billing_unit"] })}><option value="PER_SECOND">按秒计费（默认）</option><option value="PER_REQUEST">按次计费</option></select></label><label>可选时长（秒，逗号分隔）<input value={form.video_duration_options} onChange={(event) => setForm({ ...form, video_duration_options: event.target.value })} placeholder="例如 5, 10, 15；留空不限制" /></label></div>}
    <div className="three-columns"><label>{form.capability === "VIDEO_GENERATION" && form.billing_unit === "PER_SECOND" ? "每秒消耗积分数（整数）" : "每次消耗积分数（整数）"}<input type="number" min="1" max="100000" value={form.credit_cost} onChange={(event) => setForm({ ...form, credit_cost: Number(event.target.value) })} required /></label><label>模型积分系数<input type="number" min="0.000001" max="1000" step="0.000001" value={form.credit_multiplier} onChange={(event) => setForm({ ...form, credit_multiplier: Number(event.target.value) })} required /></label><label>支持参考图数量<input type="number" min="0" max="255" value={form.max_reference_images} onChange={(event) => setForm({ ...form, max_reference_images: Number(event.target.value) })} required /></label></div>
    ...
    {(form.capability === "IMAGE_GENERATION" || form.capability === "VIDEO_GENERATION") && <section className="resolution-editor"><header><div><strong>分辨率积分定价</strong><small>{form.capability === "VIDEO_GENERATION" ? form.billing_unit === "PER_REQUEST" ? "每个价格是该清晰度的一次生成费用，不乘时长" : "每个价格按生成视频秒数计费" : "每个价格按生成图片张数计费"}</small></div><button type="button" className="secondary" onClick={() => setForm({ ...form, resolution_prices: [...form.resolution_prices, { resolution: "", credit_cost: form.credit_cost }] })}>增加分辨率</button></header>{form.resolution_prices.map((price, index) => <div className="resolution-row" key={index}><label>分辨率<input value={price.resolution} placeholder={form.capability === "VIDEO_GENERATION" ? "例如 720p" : "例如 1K"} onChange={(event) => setForm({ ...form, resolution_prices: form.resolution_prices.map((item, itemIndex) => itemIndex === index ? { ...item, resolution: event.target.value } : item) })} required /></label><label>{form.capability === "VIDEO_GENERATION" ? form.billing_unit === "PER_REQUEST" ? "每次积分" : "每秒积分" : "每张积分"}<input type="number" min="1" max="100000" value={price.credit_cost} onChange={(event) => setForm({ ...form, resolution_prices: form.resolution_prices.map((item, itemIndex) => itemIndex === index ? { ...item, credit_cost: Number(event.target.value) } : item) })} required /></label><button type="button" className="secondary" onClick={() => setForm({ ...form, resolution_prices: form.resolution_prices.filter((_, itemIndex) => itemIndex !== index) })}>删除</button></div>)}</section>}
