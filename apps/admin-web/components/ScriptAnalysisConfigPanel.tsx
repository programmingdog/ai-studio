...
type ScriptAnalysisConfig = { prompt: string; credit_cost: number; extraction_billing_mode: ExtractionBillingMode; revision: number; updated_at?: string };
...
function ScriptAnalysisSetting({ token, mode }: { token: string; mode: "prompt" | "pricing" }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setConfig(result); setPrompt(result.prompt); setCreditCost(result.credit_cost); setExtractionBillingMode(result.extraction_billing_mode || "OVERALL"); setError("");
  ...
  const save = async (event: FormEvent) => {
    ...
    try {
      const result = await apiRequest<ScriptAnalysisConfig>("/admin/configs/script-analysis", { method: "PATCH", body: JSON.stringify({ prompt, credit_cost: creditCost, extraction_billing_mode: extractionBillingMode, revision: config.revision }) }, token);
      setConfig(result); setPrompt(result.prompt); setCreditCost(result.credit_cost); setExtractionBillingMode(result.extraction_billing_mode || "OVERALL"); setMessage(mode === "prompt" ? "剧本提取提示词已保存" : "剧本与视频提取定价已保存");
