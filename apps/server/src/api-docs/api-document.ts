...
const schemas: Record<string, JsonSchema> = {
  ...
  Model: {
    ...
    properties: {
      ...
      video_duration_options: { type: "array", items: { type: "integer", minimum: 1, maximum: 3600 }, description: "视频模型允许的生成秒数；空数组表示不限制" },
      aspect_ratio_options: { type: "array", items: { type: "string", enum: ["9:16", "16:9", "3:4", "4:3", "1:1"] }, description: "客户端可选择的模型画面比例；空数组时使用客户端默认值" },
      ...
      recommended: { type: "boolean", description: "是否由后台模型路由标记为推荐模型" },
      sort_order: { type: "integer", description: "后台模型路由设置的客户端展示顺序" },
  ...
  DefaultModelConfigRequest: {
    ...
    properties: {
      ...
      recommended_image_model_id: { type: "string", format: "uuid", nullable: true },
      recommended_video_model_id: { type: "string", format: "uuid", nullable: true },
    },
  },
  ...
  ProviderModelRequest: {
    ...
    properties: {
      ...
      credit_cost: { type: "integer", minimum: 0, description: "模型基础积分价格；视频由 billing_unit 决定按秒或按次" },
      billing_unit: { type: "string", enum: ["PER_SECOND", "PER_REQUEST"], default: "PER_SECOND", description: "仅视频模型可选；按次计费时不乘生成秒数" },
      video_duration_options: { type: "array", maxItems: 60, uniqueItems: true, items: { type: "integer", minimum: 1, maximum: 3600 }, description: "仅视频模型可配，例如 [5,10,15]；空数组表示不限制" },
      credit_multiplier: { type: "number", minimum: 0.000001, maximum: 1000, default: 1, description: "模型单价=消耗积分数×模型系数，带小数时向上取整；按秒视频总价再乘时长" },
      ...
      resolution_prices: { type: "array", description: "图片按张；视频按 billing_unit 决定按秒或按次的分辨率价格", items: { type: "object", required: ["resolution", "credit_cost"], properties: { resolution: { type: "string" }, credit_cost: { type: "integer", minimum: 1 } } } },
    },
  },
  ModelTestRequest: {
