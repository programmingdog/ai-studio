...
export function InvitationCard({ userId }: { userId: string }) {
  ...
  return <section className="invitation-card"><h3>邀请好友</h3>
    ...
    {!summary.data ? <p>正在读取邀请信息…</p> : <>
      <p>每邀请一位新用户完成注册，赠送你 {summary.data.invitation_reward_credits} 积分。好友先通过邀请页注册绑定，再下载安装客户端。</p>
...
function WithdrawalForm({ summary }: { summary: ReferralSummary }) {
  ...
  const chooseImage = (file?: File) => {
    ...
    if (!file) return;
    if (!["image/png", "image/jpeg"].includes(file.type) || file.size > 262144) { setFileError("请选择不超过 256KB 的 PNG/JPEG 支付宝收款码"); return; }
    setReading(true);
  ...
  return <form className="withdrawal-form" onSubmit={submit}><h3><Wallet size={18} />申请提现</h3><p>每周五北京时间 00:00～24:00 开放，最低 {money(summary.minimum_withdrawal_fen)}。申请后冻结金额，由管理员审核并通过支付宝手动打款。</p>
    ...
    <fieldset disabled={mutation.isPending}>
      ...
      <label>支付宝收款码（PNG/JPEG，≤256KB）<input ref={fileInput} type="file" accept="image/png,image/jpeg" onChange={e => chooseImage(e.target.files?.[0])} /></label>
