...
export function WorkflowStartModal({ mode, onModeChange, planned, onCancel, onStart, onCreditsPurchased, busy, error, restart = false, services = defaultServices }: {
  mode: "fast" | "storyboard"; onModeChange: (mode: "fast" | "storyboard") => void; planned: PlannedMedia[];
  onCancel: () => void; onStart: (choice: WorkflowStartChoice) => void; onCreditsPurchased?: () => void; busy: boolean; error?: string; restart?: boolean;
  services?: typeof defaultServices;
}) {
  const imageModels=useQuery({queryKey:["workflow-start-models","IMAGE_GENERATION"],queryFn:()=>services.listMediaModels("IMAGE_GENERATION")});
  const videoModels=useQuery({queryKey:["workflow-start-models","VIDEO_GENERATION"],queryFn:()=>services.listMediaModels("VIDEO_GENERATION")});
  ...
  const imageId=singleOptionValue(imageModels.data?.map(model=>model.id)??[],selectedImageId);
  const videoId=singleOptionValue(videoModels.data?.map(model=>model.id)??[],selectedVideoId);
  const image=imageModels.data?.find(m=>m.id===imageId); const video=videoModels.data?.find(m=>m.id===videoId);
  ...
  const purchaseRequired=insufficient||isInsufficientCreditError(error);
  const ready=Boolean(image?.resolution_prices.some(p=>p.resolution===imageRes)&&video?.resolution_prices.some(p=>p.resolution===videoRes)&&!imageModels.isFetching&&!videoModels.isFetching&&quotes.data&&!quotes.isFetching&&!quotes.error&&balance.data&&!balance.error&&!insufficient&&!busy);
  const picker=(kind:"image"|"video")=>{
    const query=kind==="image"?imageModels:videoModels; const list=query.data??[]; const model=kind==="image"?image:video; const res=kind==="image"?imageRes:videoRes;
    const title=kind==="image"?"图片生成方案":"视频生成方案"; const loading=query.isPending||(query.isFetching&&!query.data);
    return <section className="workflow-model-choice"><h3>{title}</h3>{loading?<div className="workflow-model-loading" role="status" aria-live="polite"><LoaderCircle className="spin" size={22}/><span><strong>正在读取{title}…</strong><small>正在获取可用大模型和清晰度</small></span></div>:query.error&&!query.data?<p className="workflow-model-message error">暂时无法加载{title}，请稍后再试。</p>:list.length===0?<p className="workflow-model-message">暂无可用的{title}</p>:<><label>选择方案<select disabled={busy} value={model?.id??""} onChange={e=>{const m=list.find(v=>v.id===e.target.value);if(kind==="image"){setImageId(e.target.value);setImageRes(m?.resolution_prices[0]?.resolution??"");}else{setVideoId(e.target.value);setVideoRes(m?.resolution_prices[0]?.resolution??"");}}}><option value="">请选择</option>{list.map(m=><option key={m.id} value={m.id}>{m.model_alias}{m.recommended ? "（推荐）" : ""}</option>)}</select></label><label>清晰度<select disabled={busy||!model} value={res} onChange={e=>kind==="image"?setImageRes(e.target.value):setVideoRes(e.target.value)}><option value="">请选择</option>{model?.resolution_prices.map(p=><option key={p.resolution} value={p.resolution}>{p.label || p.resolution} · {creditText(p.credit_cost)} 积分/{kind==="image"?"张":model?.billing_unit==="PER_REQUEST"?"次":"秒"}</option>)}</select></label>{model?.generation_notice && <small>{model.generation_notice}</small>}{model?.video_duration_options?.length ? <small>可选时长：{model.video_duration_options.join("、")} 秒</small> : null}</>}</section>;
  };
