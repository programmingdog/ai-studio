...
export function ModelCreditNotice({ capability, action = capability === "VIDEO_UNDERSTANDING" ? "视频解析" : capability === "VIDEO_REMIX" ? "二创" : "内容生成" }: { capability: "TEXT_GENERATION" | "VIDEO_UNDERSTANDING" | "VIDEO_REMIX"; action?: string }) {
  ...
  return <p className="credit-operation-notice"><Coins size={16} /><span>{quote.data
    ? `${creditNotice(action, quote.data.credits)}${capability === "VIDEO_UNDERSTANDING" ? quote.data.extraction_billing_mode === "PER_SEGMENT" ? " 长视频按分段次数分别扣费。" : " 长视频默认整体只扣 1 次。" : capability === "VIDEO_REMIX" ? " 只确认一次；失败会静默重试 3 次，失败尝试不扣分。" : ""}`
...
export function CreditConfirmationDialog({ items, available, balanceError = false, busy = false, submitError, onPurchased, onDecision }: {
  items: PendingCredit[]; available?: number; balanceError?: boolean; busy?: boolean; submitError?: string; onPurchased?: () => void; onDecision: (approved: boolean) => void;
}) {
  ...
  const purchaseRequired = insufficient || isInsufficientCreditError(submitError);
  const remixOnly = items.length > 0 && items.every(item => item.quote.capability === "VIDEO_REMIX");
  ...
  return <div className="modal-backdrop credit-confirmation-backdrop"><section className="credit-confirmation-modal" role="dialog" aria-modal="true" aria-labelledby="credit-confirmation-title">
    <header><Coins size={26} /><div><span className="eyebrow">开始前确认</span><h2 id="credit-confirmation-title">{total === 0 ? "本次免费，不扣积分" : `本次${action}需要扣 ${creditText(total)} 积分`}</h2><p>{remixOnly ? "只需确认一次；生成失败会在后台自动重试，期间不再弹窗。取消不扣分。" : "确认后开始，取消不扣分。"}</p></div></header>
    ...
    <div className="credit-confirmation-summary">{items.length > 1 && <strong>本次共 {items.length} 项，合计 {creditText(total)} 积分</strong>}<span>剩余可用积分：{available !== undefined ? `${creditText(available)} 分` : "正在查看积分…"}</span>{remixOnly ? <><p>只有二创成功后才扣分；失败尝试不扣分，系统最多自动重试 3 次。</p><p>全部自动重试失败后，手动重启任务时才会再次确认。</p></> : <>{total > 0 && <p>{creditRefundCopy}</p>}<p>{creditRetryCopy}</p></>}{purchaseRequired && <div className="insufficient-credit-callout"><div className="error-banner">积分不够了，请先购买积分套餐。</div><ImmediateCreditPurchaseButton onPurchased={onPurchased} /></div>}{balanceError && <div className="error-banner">暂时查不到剩余积分，请稍后再试。</div>}{submitError && !purchaseRequired && <div className="error-banner">操作未完成，请稍后再试。</div>}</div>
