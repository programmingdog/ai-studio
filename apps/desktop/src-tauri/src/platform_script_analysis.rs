...
pub async fn analyze_file(
    ...
    let bytes = tokio::fs::read(path).await.map_err(|error| format!("无法读取剧本文件：{error}"))?;
    let name = path.file_name().and_then(|value| value.to_str()).unwrap_or("script.txt").to_owned();
