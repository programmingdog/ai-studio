...
fn open(app: &AppHandle) -> Result<Connection, String> {
    ...
    if !has_run_id {
        connection
            .execute(
                "ALTER TABLE script_analysis_tasks ADD COLUMN run_id TEXT NOT NULL DEFAULT ''",
                [],
            )
            .map_err(|error| error.to_string())?;
    }
...
fn row(row: &rusqlite::Row<'_>) -> rusqlite::Result<ScriptAnalysisTask> {
    ...
    Ok(ScriptAnalysisTask {
        ...
        requested_project_name: row.get(5)?,
        status: row.get(6)?,
        progress: row.get(7)?,
        stage: row.get(8)?,
        message: row.get(9)?,
        expected_credits: row.get(10)?,
...
fn project_title(task: &ScriptAnalysisTask, analysis: &Value) -> String {
    ...
    if !requested.is_empty() {
        return requested.chars().take(80).collect();
    }
    let extracted = analysis
        ...
        .unwrap_or("")
        .trim();
    if !extracted.is_empty() && extracted != "未命名剧本" {
        return extracted.chars().take(80).collect();
    }
    Path::new(&task.source_name)
        .file_stem()
...
fn create_project_from_analysis(
    app: &AppHandle,
    ...
    if let Some(object) = spec.as_object_mut() {
        object.remove("platform_api_base_url");
    }
    spec["project_name"] = json!(title);
    spec["input_type"] = json!("SCRIPT");
    let duration: f64 = analysis
        .get("shots")
...
fn spawn(app: AppHandle, id: String) {
    tauri::async_runtime::spawn(async move {
        let Some(task) = find(&app, &id).ok().flatten() else {
            return;
        };
        if !Path::new(&task.source_path).is_file() {
            let _ = fail(&app, &id, "原剧本文件已不存在，请重新选择剧本");
            return;
        }
        let _ = update(
            &app,
        ...
        let idempotency = {
            let connection = match open(&app) {
                Ok(value) => value,
                Err(error) => {
                    let _ = fail(&app, &id, &error);
                    return;
                }
            };
            connection
                .query_row(
                    "SELECT idempotency_key FROM script_analysis_tasks WHERE id=?1",
                    [&id],
        ...
        let mut request = Box::pin(crate::platform_script_analysis::analyze_file(
            task.creation_spec
                .get("platform_api_base_url")
                .and_then(Value::as_str)
                .or(None),
            Path::new(&task.source_path),
            task.expected_credits,
            &idempotency,
        ));
        ...
        let value = match result {
            Ok(value) => value,
            Err(error) => {
                let _ = fail(&app, &id, &readable_error(error));
                return;
            }
        };
        let Some(analysis) = value.get("analysis").cloned() else {
            let _ = fail(&app, &id, "服务端没有返回完整的剧本提取结果");
            return;
        };
        let _ = update(
            &app,
        ...
        match project {
            ...
            Err(error) => {
                let _ = fail(&app, &id, &format!("创建项目线程失败：{error}"));
            }
        }
    });
}
...
fn validate_source(path: &str) -> Result<String, String> {
    ...
    if !path.is_file() {
        return Err("请选择存在的剧本文件".to_owned());
    }
    let extension = path
        .extension()
        .and_then(|value| value.to_str())
    ...
    if !matches!(extension.as_str(), "txt" | "md" | "docx" | "pdf") {
        return Err("仅支持 TXT、MD、DOCX 或 PDF 剧本文件".to_owned());
    }
    Ok(path
        .file_name()
        .and_then(|value| value.to_str())
...
#[tauri::command]
pub fn create_script_analysis_task(
    app: AppHandle,
    input: CreateScriptAnalysisTaskInput,
) -> Result<ScriptAnalysisTask, String> {
    let source_name = validate_source(&input.source_path)?;
    if input.root_path.trim().is_empty() {
        return Err("请选择项目保存目录".to_owned());
    }
    if !input.expected_credits.is_finite() || input.expected_credits < 0.0 {
        return Err("剧本提取积分无效".to_owned());
    }
    let id = uuid::Uuid::new_v4().to_string();
    let user_id = crate::platform_session::current_user_id()?;
    let now = Utc::now().to_rfc3339();
    let requested_project_name = input
        .creation_spec
        .get("project_name")
        .and_then(Value::as_str)
        .unwrap_or("")
    ...
    if let Some(base_url) = input
        ...
        .filter(|value| !value.trim().is_empty())
    {
        spec["platform_api_base_url"] = json!(base_url);
    }
    let connection = open(&app)?;
...
pub fn reanalyze_script_task(
    ...
    if ["PENDING", "UPLOADING", "RUNNING", "POST_PROCESSING"].contains(&task.status.as_str()) {
        return Err("该剧本正在分析中".to_owned());
    }
    create_script_analysis_task(
        app,
        CreateScriptAnalysisTaskInput {
            source_path: task.source_path,
            root_path: task.root_path,
            creation_spec: task.creation_spec,
            expected_credits,
            platform_api_base_url,
        },
    )
...
pub fn delete_script_analysis_task(app: AppHandle, task_id: String) -> Result<(), String> {
    let task = find(&app, &task_id)?.ok_or("剧本分析记录不存在")?;
    if ["PENDING", "UPLOADING", "RUNNING", "POST_PROCESSING"].contains(&task.status.as_str()) {
        return Err("分析进行中，暂时不能删除记录".to_owned());
    }
    let connection = open(&app)?;
    let user_id = crate::platform_session::current_user_id()?;
    connection
        .execute(
            "DELETE FROM script_analysis_tasks WHERE id=?1 AND user_id=?2",
