...
pub fn error(message: &str) -> String {
    json!({"code":"WORKFLOW_CREDIT_STOPPED", "message":message, "retryable":false}).to_string()
}
fn open(root: &Path) -> Result<rusqlite::Connection, String> {
    ...
    connection
        .execute_batch(
            "CREATE TABLE IF NOT EXISTS workflow_media_receipts (
...
pub fn recoverable_images(root: &Path) -> Result<Vec<String>, String> {
    ...
    let mut statement = connection
        .prepare(
            "SELECT t.id FROM image_generation_tasks t
        JOIN workflow_media_receipts r ON r.attempt=t.id
        WHERE t.status='FAILED' AND r.response_json IS NOT NULL AND r.user_id=?1",
        )
        .map_err(|e| e.to_string())?;
    let rows = statement
        .query_map([crate::platform_session::current_user_id()?], |row| {
            row.get(0)
        })
        .map_err(|e| e.to_string())?;
...
pub fn save_response(root: &Path, attempt: &str, response: &Value) -> Result<(), String> {
    let changed = open(root)?
        .execute(
            "UPDATE workflow_media_receipts SET response_json=?1 WHERE attempt=?2 AND user_id=?3",
            rusqlite::params![
                response.to_string(),
                attempt,
        ...
        .map_err(|e| e.to_string())?;
    if changed != 1 {
        return Err(error("生成结果暂时无法保存，已停止后续制作。"));
    }
    Ok(())
}
...
pub fn failure_message(root: &Path, id: &str, key: &str, attempt: &str, message: &str) -> String {
    if serde_json::from_str::<Value>(message)
        .ok()
        .is_some_and(|value| value["code"] == "WORKFLOW_CREDIT_STOPPED")
    {
    ...
    let check = update(root, id, |budget| {
        if budget
            .items
            .get(key)
            .is_some_and(|item| item.used && item.attempt.as_deref() == Some(attempt))
        {
            return Err(error(
                "这张图片的结果还需要确认，自动制作已停止，不会重新生成或重复扣分。",
            ));
        }
        Ok(())
    });
    match check {
        Ok(()) => message.to_owned(),
        Err(_) => error("这张图片的结果还需要确认，自动制作已停止，不会重新生成或重复扣分。"),
    }
}

#[tauri::command]
pub async fn approve_workflow_credit(
    project_path: String,
    ...
    for item in &items {
        ...
        {
            return Err(error("费用信息不完整，请重新开始。"));
        }
        if approved.insert(item.key.clone(), item.clone()).is_some() {
    ...
    let server_approval_id = response["approval_id"]
        ...
        .ok_or_else(|| error("服务端没有返回自动制作报价编号，请重新开始。"))?
        .to_owned();
    let returned = response["items"]
        .as_array()
        .ok_or_else(|| error("服务端返回的自动制作报价无效，请重新开始。"))?;
    let returned_by_key = returned
        .iter()
        .filter_map(|quote| quote["key"].as_str().map(|key| (key, quote)))
        .collect::<BTreeMap<_, _>>();
    approved.clear();
    for mut item in items {
        let quote = returned_by_key
            .get(item.key.as_str())
            .ok_or_else(|| error("服务端返回的自动制作报价不完整，请重新开始。"))?;
        if let Err(message) = validate(&item, quote) {
            crate::logging::error(
                "workflow.quote_mismatch",
                json!({"item_key":item.key,"original":item,"current":quote}),
            );
            let _ =
                crate::platform_media::stop_workflow_quote(&api_base, &server_approval_id).await;
            return Err(message);
        }
        item.used = false;
        item.attempt = None;
    ...
    if crate::platform_session::current_user_id()? != user {
        return Err(error("账户已切换，请重新开始。"));
    }
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let data = serde_json::to_string(&Budget {
            ...
            items: approved,
            api_base,
            server_approval_id: Some(server_approval_id),
        })
        .map_err(|e| e.to_string())?;
        connection
            .execute(
                "INSERT INTO workflow_credit_approvals(id,data) VALUES (?,?)",
                rusqlite::params![stored_id, data],
            )
            .map_err(|e| e.to_string())?;
        Ok::<_, String>(())
    })
...
fn validate(item: &BudgetItem, quote: &Value) -> Result<(), String> {
    if quote["provider_model_id"].as_str() != Some(&item.provider_model_id)
        || quote["capability"].as_str() != Some(&item.capability)
        || quote["resolution"].as_str() != Some(&item.resolution)
        || quote["seconds"].as_f64() != item.seconds
        || quote["credits"].as_f64() != Some(item.credits)
    {
        return Err(error(
            "所需积分或生成内容有变化，自动制作已停止，没有追加扣分。请重新开始。",
        ));
    }
    Ok(())
}
...
fn update(
    root: &Path,
    id: &str,
    change: impl FnOnce(&mut Budget) -> Result<(), String>,
) -> Result<(), String> {
    ...
    let data: String = tx
        .query_row(
            "SELECT data FROM workflow_credit_approvals WHERE id=?",
            [id],
            |row| row.get(0),
        )
        .map_err(|_| error("本次自动制作的积分确认已失效，请重新开始。"))?;
    let mut budget: Budget = serde_json::from_str(&data).map_err(|e| e.to_string())?;
    if budget.user != crate::platform_session::current_user_id()? {
        return Err(error("账户已切换，自动制作已停止。"));
    }
    change(&mut budget)?;
    tx.execute(
        "UPDATE workflow_credit_approvals SET data=? WHERE id=?",
        rusqlite::params![
...
pub fn reserve_legacy(
    ...
    update(root, id, |budget| {
        if !budget.active {
            return Err(error("自动制作已停止，不再扣分。"));
        }
        if !budget.items.contains_key(key) {
            return Err(missing_item_error(budget, key));
        }
        let item = budget
            .items
            .get_mut(key)
        ...
        validate(item, quote)?;
        if item.used && item.attempt.as_deref() != Some(attempt) {
            return Err(error(
                "这项内容已经提交，暂时无法确认结果。为避免重复扣分，自动制作已停止。",
            ));
        }
        item.used = true;
        item.attempt = Some(attempt.to_owned());
        Ok(())
    })
}
#[cfg(test)]
...
pub fn reserve_locked(
    ...
    seconds: Option<f64>,
    attempt: &str,
) -> Result<Option<WorkflowReservation>, String> {
    let mut reservation = None;
    update(root, id, |budget| {
        let Some(approval_id) = budget.server_approval_id.clone() else {
            return Ok(());
        };
        if !budget.active {
            return Err(error("自动制作已停止，不再扣分。"));
        ...
        let item = budget
            ...
            .get_mut(key)
            .expect("item existence checked above");
        let matches = item.provider_model_id == provider_model_id
            && item.capability == capability
            && item.resolution.eq_ignore_ascii_case(resolution)
            && match (item.seconds, seconds) {
                (None, None) => true,
        ...
        item.used = true;
        item.attempt = Some(attempt.to_owned());
        reservation = Some(WorkflowReservation {
            approval_id,
...
pub fn release(root: &Path, id: &str, key: &str, attempt: &str) -> Result<(), String> {
    update(root, id, |budget| {
        if let Some(item) = budget.items.get_mut(key) {
            if item.attempt.as_deref() == Some(attempt) {
                item.used = false;
                item.attempt = None;
            }
        }
...
pub async fn stop_workflow_credit(project_path: String, id: String) -> Result<(), String> {
    let remote = tauri::async_runtime::spawn_blocking(move || {
        let mut remote = None;
        update(Path::new(&project_path), &id, |budget| {
            budget.active = false;
            if let Some(approval) = budget.server_approval_id.clone() {
                remote = Some((budget.api_base.clone(), approval));
            }
            Ok(())
        })?;
        Ok::<_, String>(remote)
    ...
    if let Some((api_base, approval_id)) = remote {
        if let Err(message) =
            crate::platform_media::stop_workflow_quote(&api_base, &approval_id).await
        {
            crate::logging::error(
...
mod tests {
    ...
    fn request_receipts_survive_reopen_and_preserve_the_original_request() {
        ...
        let saved = receipt(&root, "attempt", "https://example.test")
            .unwrap()
            .unwrap();
        ...
        assert_eq!(
            receipt(&root, "attempt", "https://example.test")
                .unwrap()
                .unwrap()
                .response,
            Some(response)
        );
        assert!(receipt(&root, "attempt", "https://other.test")
            .unwrap()
            .is_none());
    ...
    fn local_failure_cannot_authorize_another_paid_attempt() {
        ...
        assert_eq!(
            failure_message(&root, "grant", "image:scene:S1", "attempt", &original),
            original
        );
        assert_eq!(
            failure_message(
        ...
        .contains("WORKFLOW_CREDIT_STOPPED"));
        release(&root, "grant", "image:scene:S1", "attempt").unwrap();
        assert_eq!(
            failure_message(&root, "grant", "image:scene:S1", "attempt", "refunded"),
            "refunded"
        );
        std::fs::remove_dir_all(root).unwrap();
    }
    fn setup() -> (std::path::PathBuf, Value) {
        ...
        let data = serde_json::to_string(&Budget {
            user: "test-user".into(),
            active: true,
            items: BTreeMap::from([(item.key.clone(), item)]),
            api_base: String::new(),
            server_approval_id: None,
        })
        .unwrap();
        open(&root)
            .unwrap()
            .execute(
                "INSERT INTO workflow_credit_approvals VALUES ('grant',?)",
                [data],
            )
            .unwrap();
        (
            root,
            json!({"provider_model_id":"model","resolution":"2K","seconds":null,"credits":2.5,"capability":"IMAGE_GENERATION"}),
        )
    }
    #[test]
    ...
    fn changed_price_parameters_and_unplanned_items_fail_closed() {
        ...
        for (field, value) in [
            ("credits", json!(2.6)),
            ("resolution", json!("4K")),
            ("provider_model_id", json!("other")),
            ("seconds", json!(5)),
            ("capability", json!("VIDEO_GENERATION")),
        ] {
