...
pub fn cancelled(message: &str) -> String {
    json!({"code":"CREDIT_CONFIRMATION_CANCELLED", "message":message, "retryable":false})
        .to_string()
}
...
pub async fn confirm(operation: &str, quote: Value) -> Result<Value, String> {
    ...
    pending()
        .lock()
        .map_err(|_| "积分确认队列不可用")?
        .insert(id.clone(), Pending { view, sender });
    ...
    if !matches!(approved, Ok(Ok(true))) {
        return Err(cancelled("已取消或超时，本次没有开始，不扣积分。"));
    }
    if crate::platform_session::current_user_id()? != user_id {
        return Err(cancelled("账户已切换，请重新确认积分费用"));
    }
    Ok(quote)
}
...
pub async fn list_credit_confirmations() -> Result<Vec<Confirmation>, String> {
    tauri::async_runtime::spawn_blocking(list_pending)
        .await
...
pub async fn resolve_credit_confirmation(id: String, approved: bool) -> Result<(), String> {
    tauri::async_runtime::spawn_blocking(move || resolve_pending(id, approved))
        .await
        .map_err(|error| error.to_string())?
}
...
fn resolve_pending(id: String, approved: bool) -> Result<(), String> {
    ...
    if let Some(item) = queue.get(&id) {
        if item.view.user_id != user {
            return Err("不能确认其他账户的费用".into());
        }
    }
    if let Some(item) = queue.remove(&id) {
    ...
    fn requests_wait_for_explicit_approval_and_cancellation_never_approves() {
        ...
        rt.block_on(async {
            ...
            for approved in [true, false] {
                let task = tokio::spawn(confirm(
                    "二创测试",
                    json!({"credits": 1.25, "model_alias": "测试模型"}),
                ));
