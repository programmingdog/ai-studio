...
pub(crate) fn refresh_api_base_url(configured: &str) -> Result<String, String> {
    ...
    if base == PRODUCTION_API_BASE_URL
        || base == DEVELOPMENT_API_BASE_URL
        || configured_base.as_deref() == Some(base.as_str())
    {
        Ok(base)
    } else {
...
pub async fn resume(
    ...
    if let Some((root, _, _)) = &workflow {
        let receipt =
            crate::workflow_credit::receipt(root, local_task_id, &base)?.ok_or_else(|| {
                crate::workflow_credit::error("登录账号与原任务不一致，自动制作已停止。")
            })?;
    ...
    let result = wait_for_result(
        ...
        &base,
        &token,
        json!({"task":{"id":remote_task_id,"status":"PROCESSING"}}),
        &workflow,
        local_task_id,
    )
    .await?;
    if let Some((root, _, _)) = &workflow {
        crate::workflow_credit::save_response(root, local_task_id, &result)
            .map_err(|error| crate::workflow_credit::error(&error))?;
    }
    Ok(result)
}
...
pub async fn confirmed_quote(
    ...
    crate::credit_confirmation::confirm(
        operation,
        quote(api_base, provider_model_id, capability, payload).await?,
    )
    .await
}
...
pub async fn recommended_video_concurrency(api_base: &str) -> Result<usize, String> {
    ...
    let value = response_value(
        client
            .get(format!("{base}/client-config/bootstrap"))
            .send()
            .await
            .map_err(|error| format!("无法读取平台并发配置：{error}"))?,
    )
    .await?;
    value["recommended_video_concurrency"]
        .as_u64()
...
pub async fn stop_workflow_quote(api_base: &str, approval_id: &str) -> Result<(), String> {
    ...
    authenticated_json_request(api_base, &path, json!({}))
        .await
        .map(|_| ())
}
...
pub async fn upload_reference_image(
    ...
    let send = |access_token: &str| {
        ...
        Ok::<_, String>(
            client
                .post(format!("{base}/tasks/reference-images"))
                .bearer_auth(access_token)
                .multipart(form)
                .send(),
        )
    };
    let mut response = send(&token)?
        .await
        .map_err(|error| format!("无法上传参考图：{error}"))?;
...
async fn authenticated_json_request(
    ...
    let mut response = send(&token)
        .await
        .map_err(|error| format!("无法连接平台 API：{error}"));
    if response
        .as_ref()
        .ok()
        .is_some_and(|value| value.status() == StatusCode::UNAUTHORIZED)
    {
        token = crate::platform_session::refresh_after_unauthorized(&base, &token).await?;
        response = send(&token)
            .await
...
fn login_required(error: &str) -> bool {
    serde_json::from_str::<Value>(error)
        .ok()
        .is_some_and(|value| value["code"] == "PLATFORM_LOGIN_REQUIRED")
}
...
async fn request_quote_authenticated(
    ...
    match request_quote(client, base, token, provider_model_id, capability, payload).await {
        Err(error) if login_required(&error) => {
            let refreshed =
                crate::platform_session::refresh_after_unauthorized(base, token).await?;
            request_quote(
                client,
                base,
                &refreshed,
...
async fn generate_request(
    ...
    let workflow_session = if workflow.is_some() {
        Some(
            read_platform_session()?
                .ok_or_else(|| crate::workflow_credit::error("请先登录后再开始自动制作。"))?,
        )
    } else {
        None
    };
    ...
    let quote = if let Some((root, id, key)) = &workflow {
        ...
        if workflow_session
            ...
            != Some(user.as_str())
        {
            return Err(crate::workflow_credit::error(
                "账户已切换，自动制作已停止。",
            ));
        }
        let resolution = payload
            .get("resolution")
            .or_else(|| payload.pointer("/params/resolution"))
            .and_then(Value::as_str)
            .unwrap_or("");
        let seconds = payload
            .get("seconds")
            .or_else(|| payload.get("duration"))
            .or_else(|| payload.pointer("/params/seconds"))
            .or_else(|| payload.pointer("/params/duration"))
            .and_then(Value::as_f64);
        if let Some(reservation) = crate::workflow_credit::reserve_locked(
            root,
            id,
            key,
            provider_model_id.unwrap_or(""),
            capability.unwrap_or_else(|| {
                if seconds.is_some() {
                    "VIDEO_GENERATION"
                } else {
                    "IMAGE_GENERATION"
            ...
            workflow_lock = Some(reservation);
            value
        } else {
    ...
    let mut token = if let Some(session) = workflow_session {
    ...
    let mut response = match sent {
        ...
        Err(e) => Err(format!("无法连接服务端生成接口：{e}")),
    };
    if response
        .as_ref()
        .err()
        .is_some_and(|error| login_required(error))
    {
        token = crate::platform_session::refresh_after_unauthorized(&base, &token).await?;
    ...
    let created = match response {
        ...
        Err(e) => {
            if let Some((root, id, key)) = &workflow {
                crate::logging::error(
                    "ai.media.submission_failed",
                    json!({"task_id":local_task_id,"request_id":request_id,"error":e}),
                );
                ...
                if failure["code"] == "TASK_NOT_SUBMITTED" && failure["retryable"].is_boolean() {
                    ...
                    if failure["retryable"] == false {
                        return Err(crate::workflow_credit::error(
                            failure["message"]
                                .as_str()
                                .unwrap_or("所选方案暂时无法使用，请重新选择。本次没有扣分。"),
                        ));
                    }
                    return Err(e);
                }
...
async fn recover_request(
    ...
    for _ in 0..5 {
        if let Ok(response) = client
            .get(format!("{base}/tasks/by-local/{request_id}"))
            .bearer_auth(token)
            .send()
            .await
        {
            if let Ok(task) = response_value(response).await {
...
async fn wait_for_result(
    ...
    let uncertain = |message: &str| {
        if workflow.is_some() {
            crate::workflow_credit::error(message)
        } else {
            message.to_owned()
        }
    };
    ...
    loop {
        match task_status(&current) {
            "SUCCEEDED" => {
                if let Some(result) = current.get("provider_response").filter(|v| !v.is_null()) {
                    return Ok(result.clone());
                }
                if missing_result {
                    return Err(uncertain(
                        "任务已经完成，但暂时取不到结果。已停止后续制作，不会重新生成或重复扣分。",
                    ));
                }
                missing_result = true;
            }
            "FAILED" | "CANCELED" => {
                if let Some((root, id, key)) = workflow {
                    verify_refund(client, base, &active_token, &current).await?;
                    crate::workflow_credit::release(root, id, key, local_task_id)?;
                }
                return Err(current
                    .pointer("/task/error_code")
                    .and_then(Value::as_str)
                    .unwrap_or("服务端生成任务失败")
                    .to_owned());
            }
        ...
        let task_id = current
            .pointer("/task/id")
            .and_then(Value::as_str)
            .ok_or_else(|| uncertain("暂时无法确认任务编号，已停止后续制作，不会重新扣分。"))?;
        #[cfg(not(test))]
        tokio::time::sleep(Duration::from_secs((5 + errors * 5).min(30))).await;
        #[cfg(test)]
        tokio::time::sleep(Duration::from_millis(1)).await;
        let mut queried = match client
            .post(format!("{base}/tasks/{task_id}/query"))
...
async fn verify_refund(
    ...
    let id = result
        .pointer("/task/id")
        .and_then(Value::as_str)
        .ok_or_else(|| crate::workflow_credit::error("暂时无法确认退款，自动制作已停止。"))?;
    let response = client
        .get(format!("{base}/tasks/{id}"))
        .bearer_auth(token)
        .send()
        .await
        .map_err(|_| crate::workflow_credit::error("暂时无法确认退款，自动制作已停止。"))?;
    let value = response_value(response)
        .await
        .map_err(|e| crate::workflow_credit::error(&e))?;
    if value["credits_released"] != true {
        return Err(crate::workflow_credit::error(
            "退款还未确认，自动制作已停止，不再继续扣分。",
        ));
    }
    Ok(())
}
...
mod tests {
    ...
    fn only_explicit_safe_rejection_allows_a_new_attempt() {
        let explicit: Value = serde_json::from_str(&platform_error(
            StatusCode::SERVICE_UNAVAILABLE,
            r#"{"code":"TASK_NOT_SUBMITTED","retryable":true,"message":"not started"}"#,
        ))
        .unwrap();
    ...
    fn transient_query_errors_only_poll_the_original_task() {
        ...
        let received = tauri::async_runtime::block_on(wait_for_result(
            &Client::new(),
            &base,
            "test",
    ...
    fn uncertain_submission_recovers_by_original_local_id() {
        let (base, server) = fixture(vec![(
            "GET /tasks/by-local/request-1 ",
            200,
            json!({"id":"original","status":"PROCESSING"}),
        )]);
        let recovered = tauri::async_runtime::block_on(recover_request(
            &Client::new(),
            &base,
            "test",
            "request-1",
    ...
    fn completed_task_without_result_stops_without_regeneration() {
        let (base, server) = fixture(vec![(
            "POST /tasks/original/query ",
            200,
            json!({"task":{"id":"original","status":"SUCCEEDED"}}),
        )]);
        let workflow = Some((
            std::path::Path::new("unused"),
            "grant",
            "image:scene:S1".to_owned(),
        ));
        let error = tauri::async_runtime::block_on(wait_for_result(
            &Client::new(),
            &base,
    ...
    fn accepts_production_https_and_local_development_urls() {
        assert_eq!(
            api_base_url("https://ai-studio.yuntianxing.net/api/v1/").unwrap(),
            PRODUCTION_API_BASE_URL
        );
        assert_eq!(
            api_base_url(DEVELOPMENT_API_BASE_URL).unwrap(),
    ...
    fn reads_platform_task_status_without_using_provider_status() {
        assert_eq!(
            task_status(
                &json!({"task": {"status": "SUCCEEDED"}, "provider_response": {"status": "processing"}})
            ),
            "SUCCEEDED"
        );
