...
test('no original dark surface remains, including rules masked by later overrides', () => {
  ...
  const accents = /^(?:\.brand-mark|nav button\.active::before|\.status-dot|\.welcome-glow|\.primary-button(?:,|$)|\.account-identity-avatar|\.asset-library-card\.selected \.asset-selection-check|\.promotion-poster-grid i|\.progress i|\.agent-run-progress b|\.generation-record-progress i|\.auto-workflow-overall b|\.bulk-video-overall-progress b|\.idea-workflow-overall b|\.idea-segment-progress i\.(?:active|completed)|\.douyin-task-state > i b|\.video-remix-progress i|\.script-analysis-progress > i b|\.free-task-placeholder b|\.creative-type-group-mark|\.director-list-shot|\.director-shot-number)$/;
