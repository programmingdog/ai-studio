...
3. 将 `bundle/nsis` 下生成的 `.nsis.zip` 和同名 `.sig` 上传到只读 HTTPS/CDN 地址。用户官网下载仍使用 `.exe`，在线升级必须使用 `.nsis.zip`。
4. 在管理后台“设定 → 版本管理”新建草稿：填写版本号、更新说明、灰度比例、更新包 HTTPS 地址，并粘贴 `.sig` 文件完整内容。
...
- 草稿可编辑、可删除；发布后版本元数据与更新包不可修改，只能停止分发（归档）。需要修复时发布更高版本，避免同一版本内容被替换。
