...
参考图生命周期修复包含 `056_task_reference_image_lifecycle.sql`，必须在启动新版 API 前执行迁移。该迁移只新增参考图及任务关联表，不修改积分或历史任务。客户端上传接口兼容，无需仅为此修复重新打包客户端。

二创长文本请求的 `UND_ERR_HEADERS_TIMEOUT` 修复在服务端：供应商连接的响应头、响应体及总等待时限一致，文本生成最多等待 12 分钟（客户端为 15 分钟），不会因底层 HTTP 客户端默认的 5 分钟限制提前失败。视频理解仍使用原来的 10 分钟时限，媒体接口保留原来的 30 分钟时限。超时属于网络请求失败，不自动重复提交或切换 Key，也不会当成“不合格剧本”重试。

**已有站点必须同步修改宝塔 Nginx 配置**：将 `/api/v1/` 的 `proxy_read_timeout` 和 `proxy_send_timeout` 从 `300s` 改成 `900s`，参考 `deploy/nginx.conf.example`；先测试配置，再重载 Nginx。仅更新代码或重新打包客户端不会修改已部署的 Nginx 配置；有 CDN/其他反向代理时也要核查其等待上限。本次超时修复无需新增数据库迁移或仅为此重打客户端，但需要安装更新后的服务端依赖并重新部署 API。

新版服务将所有上传参考图（包括视频首尾帧）与任务一起持久化：任一关联任务未结束即保留，全部成功、失败或取消后再保留一小时；未提交任务的上传一小时后回收。签名的 `expires_at` 仅限制绑定新任务，已绑定任务的下载权限由任务状态决定。数据库不可用时停止清理，不按文件年龄删除任务图片。没有明确终态的任务不会自动清理，需在后台核实供应商结果；容量不足会拒绝新上传，不会驱逐正在使用的图片。

保持 `/opt/aivs/shared/reference-images` 持久化挂载及签名密钥稳定。不要同时运行新旧 API：旧进程仍会按旧时限删除文件。升级前已经被旧版清理的图片无法恢复；旧任务没有持久化图片关联，不能自动追溯补绑，需核实后通过客户端重试重新上传。新版仍兼容升级时尚未提交任务的有效旧上传链接。
...
| deploy 达到超时上限后显示 `The operation was canceled` 或 `The job has exceeded the maximum execution time` | 不要直接重跑。先检查服务器最新 release、migration.log、备份和容器状态，再提交包含 120 分钟超时的新工作流 |
